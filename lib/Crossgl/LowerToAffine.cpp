...
#include "Crossgl/CrossglPasses.h"
...
class ConstantOpLowering : public mlir::OpRewritePattern<crossgl::ConstantOp> {
  using OpRewritePattern<crossgl::ConstantOp>::OpRewritePattern;

  mlir::LogicalResult
  matchAndRewrite(crossgl::ConstantOp op,
...
class PrintOpLowering : public mlir::OpConversionPattern<crossgl::PrintOp> {
  using OpConversionPattern<crossgl::PrintOp>::OpConversionPattern;

  mlir::LogicalResult
  matchAndRewrite(crossgl::PrintOp op, OpAdaptor adaptor,
...
namespace {
class CrossglToAffineLowerPass
    : public mlir::PassWrapper<CrossglToAffineLowerPass,
                               mlir::OperationPass<mlir::ModuleOp>> {
public:
  MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(CrossglToAffineLowerPass)
...
void CrossglToAffineLowerPass::runOnOperation() {
  mlir::ConversionTarget target(getContext());

  target.addIllegalDialect<crossgl::CrossglDialect>();
  target.addLegalDialect<mlir::affine::AffineDialect, mlir::BuiltinDialect,
                         mlir::func::FuncDialect, mlir::arith::ArithDialect,
                         mlir::memref::MemRefDialect>();
  target.addDynamicallyLegalOp<crossgl::PrintOp>([](crossgl::PrintOp op) {
    return llvm::none_of(op->getOperandTypes(), [](mlir::Type type) {
      return type.isa<mlir::TensorType>();
    });
  });
  target.addLegalOp<crossgl::WorldOp>();
...
std::unique_ptr<mlir::Pass> crossgl::createLowerToAffinePass() {
  return std::make_unique<CrossglToAffineLowerPass>();
}
