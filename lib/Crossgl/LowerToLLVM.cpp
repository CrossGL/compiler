...
public:
  explicit PrintOpLowering(mlir::MLIRContext *context)
      : mlir::ConversionPattern(crossgl::PrintOp::getOperationName(), 1,
                                context) {}
  ...
  matchAndRewrite(mlir::Operation *op, mlir::ArrayRef<mlir::Value> operands,
    ...
    auto printOp = mlir::cast<crossgl::PrintOp>(op);
    auto elementLoad =
...
public:
  explicit WorldOpLowering(mlir::MLIRContext *context)
      : mlir::ConversionPattern(crossgl::WorldOp::getOperationName(), 1,
                                context) {}
...
class CrossglToLLVMLoweringPass
    : public mlir::PassWrapper<CrossglToLLVMLoweringPass,
                               mlir::OperationPass<mlir::ModuleOp>> {
public:
  MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(CrossglToLLVMLoweringPass)
...
void CrossglToLLVMLoweringPass::runOnOperation() {
  ...
  patterns.add<crossgl::PrintOpLowering, crossgl::WorldOpLowering>(&getContext());
...
std::unique_ptr<mlir::Pass> crossgl::createLowerToLLVMPass() {
  return std::make_unique<CrossglToLLVMLoweringPass>();
}
