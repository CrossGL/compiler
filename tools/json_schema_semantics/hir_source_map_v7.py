"""Semantic checks for hir-source-map-v7.schema.json."""
...
def validate_source_map_record_context(
    errors,
    path,
    record,
    *,
    statement_record=False,
):
    if record["entryPoint"] and not record["stage"]:
        errors.append(f"{path}.stage: expected non-empty when entryPoint is non-empty")
    statement_kind = record.get("statementKind", "")
    if statement_record and not statement_kind:
        errors.append(f"{path}.statementKind: expected non-empty")
    if statement_kind and not record["function"]:
        errors.append(
            f"{path}.function: expected non-empty when statementKind is non-empty"
        )


def validate_source_map_record_contexts(errors, path, locations):
...
def validate_semantics(instance):
    ...
    validate_source_map_record_contexts(errors, "$.hirSourceLocations", locations)
    ...
    add_length_count_error(
        errors,
        "$.pagination.statementEmittedCount",
        pagination["statementEmittedCount"],
        locations["statements"],
        "$.hirSourceLocations.statements length",
    )
    ...
    add_equal_error(
        errors,
        "$.pagination.statementTotalCount",
        pagination["statementTotalCount"],
        categories["statementTotalCount"],
        "$.categoryCounts.statementTotalCount",
    )
    ...
    for kind in ["expression", "type", "statement"]:
